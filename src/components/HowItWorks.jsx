...
export default function HowItWorks() {
  ...
  return (
    <section id="sdk-start" className="py-24 bg-white">
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div ref={ref} className={`text-center mb-20 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-[17.5px] text-slate-500 mt-4 max-w-[460px] mx-auto leading-relaxed">
        ...
        <div className="relative max-w-[900px] mx-auto grid md:grid-cols-3 gap-10">
          ...
          {STEPS.map((s, i) => (
            ...
            >
              ...
              <span className="inline-block px-3 py-1 bg-blue-50 text-brand text-[11px] font-bold uppercase tracking-widest rounded-full mb-3">
                {s.tag}
              </span>
              <h3 className="text-[19px] font-bold text-slate-900 mb-2">{s.title}</h3>
              <p className="text-[14.5px] text-slate-500 leading-relaxed">{s.desc}</p>
            </div>
          ))}
        </div>
